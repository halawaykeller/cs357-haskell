...
minimax :: Field -> Tree Board -> Tree (Board,Int)
minimax p (Node g [])
   | wins p' g  = Node (g,(-10)) []
   | wins p g  = Node (g,10) []
   | otherwise = Node (g,0) []
...
-- If the board is empty, always start in the upper right hand corner
bestmove :: Board -> Field -> Board
bestmove g p 
            | g == empty = concat (move g 0 p)
            | otherwise = head [g' | Node (g',p') _ <- ts, p' == best]
               where 
                  tree = prune depth (gametree g p)
                  Node (_,best) ts = minimax p tree

-- Testing

-- Since Green always goes second, these are the only valid starting boards for Green. 
...
-- Given a board state, returns a list of a possible valid next moves for the given player
...
-- This tests the strategy for the FIRST player - which still uses the minimax algorithm, but starts with any empty board
testRedStrategy :: [Bool]
testRedStrategy= testStrategy empty R

-- Generalize testing strategy. This takes a starting board and a player and recursively tests all 
-- games that can be played based on the best strategy for the given player. The opposing player makes every
-- possible move, then we test the strategy against each new board state until a player has won, or there is a draw.
